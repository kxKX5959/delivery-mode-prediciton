...
def loadData(dataath):
    ...
    with open('G:\华医数据临床信息\最终版/剖宫产+顺产-最新筛选.csv') as f:
        ...
        for row in reader:
            ...
            features = np.concatenate((feature2, feature3, feature4, ))
...
def EEGNet_v2(nb_classes, Samples=4800,
    ...
    dense = Dense(nb_classes, name='dense', kernel_constraint=max_norm(0.25))(features)
    # kernel_constraint=max_norm(norm_rate) 0.15
    sig = Activation('sigmoid', name='sigmoid')(dense)

    return Model(inputs=[input1,input2], outputs=sig)
...
for train, test in kfold.split(Data, Label):
    ...
    class_weights = {0: 0.29, 1: 0.71}
    fittedModel = model.fit([x_res, eng_trainf], y_res, batch_size=16, epochs=120,
                            verbose=2,
    ...
    pred_vt = model.predict([Data[test], eng_testf], batch_size=16, verbose=0)
